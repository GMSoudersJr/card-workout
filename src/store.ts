...
function createCheckboxStore() {
	...
	return {
		subscribe,
		updateCategory: (value: TExerciseCategory) =>
			update((checkedBoxes) => {
		...
		updatePosition: (value: TExercisePosition) =>
			update((checkedBoxes) => {
		...
		updateVariation: (value: TExerciseVariation) =>
			update((checkedBoxes) => {
...
function createTheDeckOfCards() {
	...
	return {
		...
		pluck: (someRandomNumber: number) =>
			update((deck) => {
				deck[someRandomNumber].hasBeenPlucked = true;
				return deck;
			}),
		discard: (someRandomNumber: number) =>
			update((deck) => {
		...
		putBack: (theDeckIndexOfTheCurrentCard: number) =>
			update((deck) => {
			...
			update((stopwatch) => {
				...
				stopwatch._intervalId = setInterval(() => {
					stopwatch.elapsedTime =
						stopwatch.paused.elapsedTime + (Date.now() - stopwatch.continuedAt);
				}, 10);

				return stopwatch;
			}),

		updateDisplayIntervalId: (displayIntervalId: number) =>
			update((stopwatch) => {
...
function createSuitExercisesStore() {
	...
	return {
		subscribe,
		updateExercise: (suit: TSuit, exercise: TExerciseName | undefined) =>
			update((result) => {
		...
		addReps: (aDiscardedCard: TPlayingCard<TCardRank, TSuit>) =>
			update((result) => {
				const theSuitOfTheDiscardedCard = aDiscardedCard.suit;
				const theValueOfTheDiscardedCard = aDiscardedCard.value;
				const ofThisSuit = (entry: TSuitExercise<TSuit>) =>
					entry.suit === theSuitOfTheDiscardedCard;
